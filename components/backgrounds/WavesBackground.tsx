...
/** Ondas de linhas reativas ao mouse (Perlin noise). Linha dourada sobre preto. */
export default function WavesBackground({
  ...
  useEffect(() => {
    ...
    const config = { lineColor: color, waveAmpX: 24, waveAmpY: 12, friction: 0.92, tension: 0.004, maxCursorMove: 80, xGap: 12, yGap: 24, waveSpeedX: 0.008, waveSpeedY: 0.003 };
    ...
    function movePoints(time: number) {
      lines.forEach((pts) => pts.forEach((p: any) => {
        ...
        p.cursor.vx += (0 - p.cursor.x)*config.tension; p.cursor.vy += (0 - p.cursor.y)*config.tension;
    ...
    function moved(point: any, withCursor = true) {
      return { x: Math.round((point.x + point.wave.x + (withCursor?point.cursor.x:0))*10)/10,
    ...
    function tick(t: number) {
      ...
      mouse.v = d; mouse.vs += (d-mouse.vs)*0.1; mouse.vs = Math.min(80, mouse.vs);
    ...
    const onTouch = (e: TouchEvent) => { const t = e.touches[0]; updateMouse(t.clientX, t.clientY); };
